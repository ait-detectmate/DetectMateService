...
- `container/config:/config` provides the DetectMate configuration. Each DetectMate Service needs one settings-file for the general service configuration and one config file for the specific DetectMate component. For detector these files are `detector_settings.yaml` for service settings and `detector_config.yaml` that holds the component-specific parameters.
