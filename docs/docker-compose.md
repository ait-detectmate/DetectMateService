...
- `container/run:/run` is mounted to hold the socket files for inter process communication. `fluentin` and `parser` are talking in this configuration using [Nano Message Next Generation](https://nng.nanomsg.org/) via unix domain sockets for maximum performance. 
